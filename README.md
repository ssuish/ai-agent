...
- **Predefined tools** — Implemented for the agent to call:
  - **Files info** (`functions/get_files_info.py`) — Lists directory contents (names, sizes, is_dir) with path-safety so the agent cannot escape the allowed working directory.
...
- `test_get_files_info.py` — tests for `get_files_info` (run from repo root).
...
- **Files info** (programmatic):
  - `get_files_info(working_directory, directory=".")`  
  - Returns a string of lines: `name : file_size=<bytes>, is_dir=<bool>`.  
  - Restricts listing to paths under `working_directory`; returns an error string if `directory` is outside it or not a directory.
...
- **get_files_info** (from repo root):
  ...
  ```
  Covers: current dir, subdir (`pkg`), and rejection of `/bin` and `../` (outside permitted working directory).
...
- **get_files_info**  
  ...
  - Returns plain strings; errors are returned as `"Error: ..."` strings rather than exceptions for caller-friendly handling.

- **Type checking**  
  - `pyrightconfig.json` sets `extraPaths: ["."]` so imports like `calculator.pkg` and `functions.get_files_info` resolve from the project root.
...
| Predefined tools | `get_files_info` (directory scanning) implemented and tested; path safety in place. |
